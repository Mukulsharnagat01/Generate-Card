...
import { Move, Upload, Menu, X } from "lucide-react";
...
const EditTemplate = () => {
  ...
  const [sizes, setSizes] = useState<{
    ...
    company: number;
    logo: number;
  }>({ name: 20, title: 15, company: 12, logo: 64 });
  ...
  const [backSizes, setBackSizes] = useState<{
    ...
    address: number;
    qr: number;
  }>({ email: 13, phone: 13, website: 13, address: 13, qr: 72 });

  // Refs for drag & drop
  const previewRef = useRef<HTMLDivElement>(null);
  const backPreviewRef = useRef<HTMLDivElement>(null);
  const dragState = useRef<{
    key: FrontKey | BackKey | null;
    offsetX: number;
    offsetY: number;
  ...
  useEffect(() => {
    const updatePreviewScale = () => {
      if (previewContainerRef.current) {
        ...
        // For better responsiveness, use dynamic calculation
        const maxCardWidth = 450; // Maximum card width
        const scale = Math.min(containerWidth / maxCardWidth, 1);
        setPreviewScale(Math.max(0.5, scale)); // Minimum scale 0.5
      }
    };
  ...
  const onBackDragStart = (
    ...
    document.addEventListener("touchmove", onBackDragMove as any, {
      passive: false,
    });
  ...
  const onBackDragMove = (e: MouseEvent | TouchEvent) => {
    if (!isEditLayout || !backPreviewRef.current || !dragState.current.key)
      return;
    e.preventDefault();
    const rect = backPreviewRef.current.getBoundingClientRect();
    const pointX = e instanceof TouchEvent ? e.touches[0]?.clientX : (e as MouseEvent).clientX;
  ...
  const onBackResizeStart = (
    ...
    document.addEventListener("touchmove", onBackResizeMove as any, {
      passive: false,
    });
  ...
  const onBackResizeMove = (e: MouseEvent | TouchEvent) => {
    ...
    const newSize = Math.max(
      clampMin,
      Math.min(clampMax, Math.round(resizeState.current.baseSize + delta * 0.2))
    );
  ...
  const resetPositions = () => {
    ...
    setSizes({ name: 20, title: 15, company: 12, logo: 64 });
    setPositionsBack({
      email: { x: 15, y: 20 },
      phone: { x: 15, y: 32 },
      website: { x: 15, y: 44 },
    ...
    setBackSizes({ email: 13, phone: 13, website: 13, address: 13, qr: 72 });
  };
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      let back_background_url =
        backBgUrlInput || item?.back_background_url || null;
...
END:VCARD`;
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="max-w-7xl mx-auto px-3 md:px-4 lg:px-6 py-4 min-h-screen bg-gray-50/50"
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        ...
        {/* Mobile Menu */}
        <div className={`${mobileMenuOpen ? 'flex' : 'hidden'} sm:flex flex-col sm:flex-row gap-3 w-full sm:w-auto`}>
          <Button
            type="button"
            variant="outline"
            onClick={() => navigate(-1)}
            className="w-full sm:w-auto justify-center py-2 px-4"
          >
            Cancel
          </Button>
          <Button
            type="submit"
            disabled={saving}
            className="w-full sm:w-auto min-w-[120px] justify-center py-2 px-4"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6">
        {/* LEFT: Sticky Preview Area (Span 5) - RESPONSIVE FIXED */}
        <div className="lg:col-span-5">
          <div className="sticky top-4 space-y-4">
            {/* Live Preview Header - NOW WITH SCROLLABLE BUTTONS */}
            <div className="bg-white p-3 rounded-lg border shadow-sm">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <span className="text-xs font-semibold uppercase tracking-wider text-gray-500 px-2 whitespace-nowrap">
                  Live Preview
                </span>
                ...
                <div className="w-full sm:w-auto overflow-x-auto scrollbar-hide">
                  <div className="flex items-center gap-2 min-w-max pb-1">
                    <Button
                      ...
                      className="text-blue-600 hover:text-blue-700 hover:bg-blue-50 text-xs sm:text-sm whitespace-nowrap shrink-0"
                    >
                      Switch to {showBack ? "Front" : "Back"}
                    </Button>
                    ...
                    <Button
                      ...
                      className="flex items-center gap-1 text-xs sm:text-sm whitespace-nowrap shrink-0"
                    >
                      <Move className="w-3 h-3" />
                      {isEditLayout ? "Exit Edit" : "Edit Layout"}
                    </Button>
                    ...
                    {/* Logo Upload Button in Preview Controls */}
                    ...
                    {previewData.logo && (
                      <Button
                        ...
                        className="text-xs whitespace-nowrap shrink-0 text-red-600 hover:text-red-700"
                      >
                        Remove Logo
                      </Button>
                    )}
            ...
            {/* The Card Render - RESPONSIVE SCALING */}
            <div 
              ...
              >
                ...
                >
                  <div className="w-full h-full">
                    {showBack ? (
                      ...
                      !isEditLayout ? (
                        ...
                        <div className="w-full h-full relative">
                          ...
                          >
                            <strong style={{ color: accentColor }}>✉</strong>{" "}
                            {previewData.email}
                            {isEditLayout && (
                              <span
                    ...
                    !isEditLayout ? (
                      <div className="w-full h-full flex items-center justify-end p-4 md:p-8">
                      ...
                      <div className="w-full h-full relative">
                        ...
                        {previewData.logo && (
                          <div
                            className="cursor-move select-none absolute bg-white rounded-full overflow-hidden shadow-lg border-4 border-white"
                            style={{
                              left: `${positions.logo.x}%`,
                              top: `${positions.logo.y}%`,
                              width: sizes.logo * previewScale,
                              height: sizes.logo * previewScale,
                  ...
                  {/* Edit Mode Indicator */}
                  {isEditLayout && (
                    <div className="absolute top-2 left-2 bg-blue-500 text-white text-[10px] sm:text-xs px-2 py-1 rounded">
                      Edit Mode
                    </div>
                  )}
                </div>
            ...
            {isEditLayout && (
              <div className="bg-yellow-50 p-4 rounded-lg text-sm text-yellow-800 border border-yellow-100">
                ...
                <ul className="mt-1 list-disc pl-5 space-y-1 text-xs">
                  ...
                  <li>Positions will be saved when you update template</li>
                </ul>
              </div>
            )}

            {/* Logo Upload Section - COMPACT VERSION */}
                ...
                {previewData.logo && (
                  <div className="flex items-center gap-2">
                    <div className="w-10 h-10 rounded-full overflow-hidden border">
                      <img src={previewData.logo} alt="Logo" className="w-full h-full object-cover" />
                    </div>
                  </div>
                )}
              </div>
        ...
        {/* RIGHT: Form Controls (Span 7) - SCROLLABLE TABS */}
        <div className="lg:col-span-7">
          <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
            ...
            <div className="flex border-b bg-gray-50/50 overflow-x-auto scrollbar-hide">
              {["general", "design", "qr", "content", "layout"].map((tab) => (
                <button
                  ...
                  className={`px-4 py-3 text-xs sm:text-sm font-medium capitalize transition-colors whitespace-nowrap min-w-[80px] flex-shrink-0 ${
                    activeTab === tab
            ...
            <div className="p-4 md:p-6 space-y-6 overflow-y-auto max-h-[calc(100vh-200px)]">
              {/* GENERAL TAB */}
              {activeTab === "general" && (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg space-y-3">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        />
                        <span className="text-xs self-center text-gray-400 hidden sm:inline">OR</span>
                        <input type="file" className="w-full sm:w-auto text-xs file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" accept="image/*" onChange={(e) => setBgFile(e.target.files?.[0] ?? null)} />
                      </div>
                      {item.background_url && !bgFile && (
                        <p className="text-xs text-gray-400 truncate">
                          Current: {item.background_url.split("/").pop()}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="p-4 bg-gray-50 rounded-lg space-y-3">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        />
                        <span className="text-xs self-center text-gray-400 hidden sm:inline">OR</span>
                        <input type="file" className="w-full sm:w-auto text-xs file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" accept="image/*" onChange={(e) => setBackBgFile(e.target.files?.[0] ?? null)} />
                      </div>
                      {item.back_background_url && !backBgFile && (
                        <p className="text-xs text-gray-400 truncate">
                          Current: {item.back_background_url.split("/").pop()}
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <option value="Verdana, sans-serif">Verdana (Highly Readable)</option>
                        <option value="Tahoma, sans-serif">Tahoma (Sharp Clean)</option>
                        <option value="Trebuchet MS, sans-serif">Trebuchet (Modern Professional)</option>
                        <option value="Georgia, serif">Georgia (Classic Serif)</option>
                        <option value="Times New Roman, serif">Times New Roman (Formal Classic)</option>
                        <option value="Garamond, serif">Garamond (Elegant Old-Style)</option>
                        <option value="Courier New, monospace">Courier New (Mono Retro)</option>
                        <option value="Lucida Console, monospace">Lucida Console (Tech Mono)</option>
                        <option value="'Brush Script MT', cursive">Brush Script (Elegant Handwriting)</option>
                        <option value="'Comic Sans MS', cursive">Comic Sans (Casual Handwriting)</option>
                      </select>
                    </div>
                  </div>
                </div>
...
//         ? e.touches[0]?.clientX ?? dragState.current.offsetX
//         : (e as MouseEvent).clientX;
//     const pointY =
//       e instanceof TouchEvent
//         ? e.touches[0]?.clientY ?? dragState.current.offsetY
...
//     const curY =
//       e instanceof TouchEvent
//         ? e.touches[0]?.clientY ?? resizeState.current.startY
//         : (e as MouseEvent).clientY;
//     const delta = curY - resizeState.current.startY;
//     const k = resizeState.current.key as FrontKey;
//     const newSize = Math.max(
//       8,
//       Math.min(64, Math.round(resizeState.current.baseSize + delta * 0.2))
//     );
//     setSizes((s) => ({ ...s, [k]: newSize }));
//     if (e instanceof TouchEvent) e.preventDefault();
//   };
...
//     window.addEventListener("touchmove", onBackDragMove as any, {
//       passive: false,
//     });
...
//     window.addEventListener("touchmove", onBackResizeMove as any, {
//       passive: false,
//     });
...
//     <form
//       onSubmit={onSubmit}
//       className="max-w-7xl mx-auto p-4 md:p-6 min-h-screen bg-gray-50/50"
//     >
//       {/* Header - Mobile Responsive */}
...
//             type="submit"
//             disabled={saving}
//             className="w-full sm:w-auto min-w-[100px] justify-center"
//           >
//             {saving ? "Saving..." : "Save"}
//           </Button>
//         </div>
...
//             <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-white p-3 rounded-lg border shadow-sm gap-3">
...
//             <div
//               ref={showBack ? backPreviewRef : previewRef}
//               className="w-full aspect-[1.75/1] max-w-[400px] mx-auto rounded-xl shadow-2xl border border-gray-200 overflow-hidden transition-all duration-500 relative"
//               style={{
//                 backgroundColor: getPreviewBg() ? undefined : "#ffffff",
...
//               <div className="bg-yellow-50 p-4 rounded-lg text-sm text-yellow-800 border border-yellow-100">
//                 <p><strong>Layout Editing Active:</strong></p>
//                 <ul className="mt-1 list-disc pl-5 space-y-1 text-xs sm:text-sm">
//                   <li>Drag any element to reposition</li>
//                   <li>Use the blue handles at bottom-right to resize</li>
//                   <li>Switch between Front/Back views</li>
//                   <li>Click "Reset Positions" to restore defaults</li>
...
//           <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
//             {/* Custom Tab Navigation - Mobile Responsive */}
...
//                   type="button"
//                   onClick={() => setActiveTab(tab)}
//                   className={`px-4 py-3 text-xs sm:text-sm font-medium capitalize transition-colors whitespace-nowrap ${activeTab === tab
//                     ? "border-b-2 border-black text-black bg-white"
//                     : "text-gray-500 hover:text-gray-700 hover:bg-gray-100"
//                     }`}
//                 >
...
//                         />
//                         <span className="text-xs self-center text-gray-400 hidden sm:inline">OR</span>
//                         <input type="file" className="w-full sm:w-1/3 text-xs" accept="image/*" onChange={(e) => setBgFile(e.target.files?.[0] ?? null)} />
//                       </div>
//                     </div>
...
//                         />
//                         <span className="text-xs self-center text-gray-400 hidden sm:inline">OR</span>
//                         <input type="file" className="w-full sm:w-1/3 text-xs" accept="image/*" onChange={(e) => setBackBgFile(e.target.files?.[0] ?? null)} />
//                       </div>
//                     </div>
//                   </div>
